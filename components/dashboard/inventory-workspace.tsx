    ...
    <>
      ...
      >
          ...
          {row.recommended_qty > 0 ? (
            <>
              ...
              <p className="mt-3 text-meta text-ink-tertiary">
                {row.moq > 0 && <>Minimum order quantity {formatNumber(row.moq)} units · </>}
        ...
        <section className="mt-5 rounded-md bg-surface-card p-4">
          ...
          <div className="mt-3">
            {row.recent_demand.length === 0 ? (
              // Present but empty: the service does not return per-series
              // history on a recommendation yet, so there is nothing to draw.
              <p className="py-3 text-body-sm text-ink-secondary">
                No recent demand returned for this series.
              </p>
            ) : (
